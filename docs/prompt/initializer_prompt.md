...
先阅读工作区内的 **`docs/SKILL-SPEC.md`**。该文件描述本仓库要落实的 Skill 与 SDK 调度层规格，**务必完整阅读后再继续**。

---

### 关键任务一：生成 `feature_list.json`

根据 **`docs/SKILL-SPEC.md`**，在工作区根目录创建 **`feature_list.json`**，作为后续实现的 **单一真相源**。
...
根据 **`docs/SKILL-SPEC.md`** 搭建 **最小可扩展目录**（例如 `src/`、`docs/`、后续 Skill 或 SDK 包路径等），不必一次实现全部逻辑，但须 **与规格中的模块划分一致**，便于后续会话增量开发。
